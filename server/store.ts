...
function stripDocs<T extends { fields?: Record<string, string> } | undefined>(obj: T): T {
  ...
  const fields = { ...f }
  for (const [k, label] of Object.entries(DOC_KEYS)) {
    if (fields[k]) fields[k] = `[${label} ${String(fields[k]).replace(/\s/g, '').length}자 제공됨 · 원문 미저장]`
  }
  return { ...obj, fields }
}
...
export async function listSessions(userKey: string, limit = 10, offset = 0): Promise<SessionRow[]> {
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT id, domain, title, score, eye_contact, duration_ms, report_status, created_at,
            JSON_EXTRACT(setup, '$.realMode') AS real_mode,
            (SELECT COUNT(*) FROM JSON_TABLE(sessions.turns, '$[*]' COLUMNS(role VARCHAR(20) PATH '$.role')) jt WHERE jt.role = 'user') AS user_turns FROM sessions
...
export async function listAllSessions(limit = 30, offset = 0) {
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT s.id, s.user_key, u.nickname, s.domain, s.title, s.score, s.eye_contact, s.duration_ms, s.report_status, s.created_at,
            ...
            (SELECT COUNT(*) FROM JSON_TABLE(s.turns, '$[*]' COLUMNS(role VARCHAR(20) PATH '$.role')) jt WHERE jt.role = 'user') AS user_turns
