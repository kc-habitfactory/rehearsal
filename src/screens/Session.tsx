...
export function Session({ setup, scenario, engine, stream, onFinish }: Props) {
  ...
  const endLabel = dom.id === 'scam_call' || dom.id === 'claim_appeal' ? '전화 끊기' : dom.id === 'insurance_consult' ? '상담 마치기' : dom.id === 'hospital' ? '진료 끝' : dom.id === 'money_talk' ? '대화 마치기' : dom.id === 'parent_teacher' ? '면담 마치기' : '종료'
